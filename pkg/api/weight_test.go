...
func (m mockWeightRepo) GetUser(userID int) (api.User, error) {
	if userID != 1 {
		return api.User{}, errors.New("storage - user doesn't exist")
	}
...
func TestCreateWeightEntry(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			name: "should return a error because user doesn't exist",
			request: api.NewWeightRequest{
				Weight: 70,
				UserID: 2,
			},
			want: errors.New("storage - user doesn't exist"),
		},
	}
...
func TestDailyIntake(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			name:          "should calculate daily intake for activity level 4 with weight loss as goal",
			WeightGoal:    "loose",
			BMR:           1441,
			ActivityLevel: 4,
			want:          1985,
			...
			name:          "should calculate daily intake for activity level 5 with weight loss as goal",
			WeightGoal:    "loose",
			BMR:           1441,
			ActivityLevel: 5,
			want:          2237,
