...
const (
	...
	// High exercise activity multiplier - 6-7x per week fro 45-60 mins - 4
	highActivity = 1.725
	// Very high exercise activity - for people who are athletes - 5
...
type WeightService interface {
	...
	DailyIntake(MBR, activityLevel int, weightGoal string) (int, error)
}
...
func (w *weightService) DailyIntake(BMR, activityLevel int, weightGoal string) (int, error) {
	...
	default:
		return 0, errors.New("invalid variable activityLevel - needs to be either 1,2,3,4 or 5")
	}
	...
	default:
		return 0, errors.New("invalid weight goal provided - must be either gain, loose or maintain")
	}
...
func (w *weightService) New(request NewWeightRequest) error {
	...
	newWeight := Weight{
		...
		UserID:             user.ID,
		BMR:                bmr,
		DailyColoricIntake: dailyIntake,
	}
